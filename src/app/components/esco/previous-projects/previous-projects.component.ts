...
export class PreviousProjectsComponent implements OnInit {
  ...
  createProjectGroup(): FormGroup {
    return this.fb.group({
      pp_client_name: ['', [Validators.required, Validators.pattern(/^[a-zA-Z ]*$/)]],
      pp_contact_person: ['', [Validators.required, Validators.pattern(/^[a-zA-Z ]*$/)]],
      pp_client_contact_no: ['', [Validators.required, Validators.pattern(/^\d{10}$/)]],
      ...
      projects.forEach((project: any, i: number) => {
        ...
        this.createService.StepSaveCompanyPreviousProjects(formData).subscribe({
          next: (res) => {
            if (res.status === 'success') {
              ...
              if (numSubmitted + numFailed === projects.length) {
                ...
                this.saveSuccessMessage = 'Previous Projects successfully saved! Click Next to add more data.';
          ...
          error: (err) => {
            numFailed++;
            this.saveSuccessMessage = null;
            this.saveErrorMessage = 'An error occurred while adding Previous Projects.';
            setTimeout(() => {
              this.saveErrorMessage = null;
            }, 3000);
          }
